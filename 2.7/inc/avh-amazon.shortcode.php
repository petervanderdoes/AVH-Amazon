...
class AVHAmazonShortcode extends AVHAmazonCore {
	...
	function shortcodeAsin ( $attrs, $content, $associatedid ) {
		...
		$error = '';
		$item_result = $this->restCall ( $this->getRestItemLookupParams ( $attrs['asin'], $associatedid ) );
	...
	function metaboxTabWishlistOutput ( $values ) {
		...
		if ( $total_items > 0 ) {
			...
			foreach ( $listitem as $key => $value ) {
				$Item = $value;
				$item_result = $this->restCall ( $this->getRestItemLookupParams ( $Item['Item']['ASIN'], '' ) );
	...
	function metaboxTabAsinOutput ( $values ) {
		...
		$item_result = $this->restCall ( $this->getRestItemLookupParams ( $asin, '' ) );
		if ( $item_result['Items']['Request']['Errors'] ) {
